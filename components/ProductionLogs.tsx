...
import { Plus, Trash2, Calendar, Package, Hash, Calculator, TrendingUp, DollarSign, BadgeDollarSign, Search, ChevronDown, X, FileCode, CheckCircle2, AlertCircle, Loader2, Sparkles, Save, ShoppingBag, ReceiptText, Pencil, Files } from 'lucide-react';
...
interface Props {
  ...
  onAddProductsBatch?: (products: Omit<Product, 'id'>[]) => Promise<Product[]>;
  onAddLogsBatch?: (logs: Omit<ProductionLog, 'id'>[]) => Promise<void>;
...
const ProductionLogs: React.FC<Props> = ({ products, categories, logs, onAdd, onUpdate, onDelete, onAddProduct, onAddProductsBatch, onAddLogsBatch, onAddCategory, onTogglePaid }) => {
  ...
  const [allEntriesToLog, setAllEntriesToLog] = useState<LogEntry[]>([]);
  ...
  const processFile = (file: File): Promise<{ entries: LogEntry[], newProducts: PendingProduct[] }> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (event) => {
        try {
          ...
          resolve({ entries: fileEntries, newProducts: fileNewProducts });
    ...
    try {
      ...
      results.forEach(res => {
        ...
        res.newProducts.forEach(newP => {
          const nameKey = newP.name.trim().toLowerCase();
          if (!combinedNewProducts.find(p => p.name.trim().toLowerCase() === nameKey)) {
            combinedNewProducts.push(newP);
          }
        });
      });
      ...
      if (combinedNewProducts.length > 0) {
        ...
        if (onAddLogsBatch) {
          const logsToSave = combinedEntries.map(e => ({
        ...
        alert(`Sucesso! ${combinedEntries.length} lançamentos realizados.`);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.productId || isImporting) return;
    ...
    try {
      if (editingLogId) { await onUpdate(logData); setEditingLogId(null); }
      else { await onAdd(logData); }
  ...
  const handleEditClick = (log: ProductionLog) => {
    ...
    setFormData({ productId: log.productId, date: log.date, quantity: log.quantity.toString(), invoiceNumber: log.invoiceNumber || '' });
    ...
    <div className="space-y-4 animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-6">
          ...
          {!editingLogId && (
            <button 
              ...
              className="bg-emerald-50 text-emerald-700 px-4 py-2 rounded-lg text-xs font-bold border border-emerald-200 flex items-center gap-2 hover:bg-emerald-100 transition-all shadow-sm group"
            >
              ...
              Importar XMLs (Vários Arquivos)
              <input type="file" accept=".xml" multiple className="hidden" ref={fileInputRef} onChange={handleImportXML} />
            </button>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end">
          ...
          <div className="flex gap-2 md:col-span-5 lg:col-span-1">
            <button type="submit" disabled={!formData.productId || isImporting} className={`flex-1 font-bold py-2 rounded-xl text-xs h-[38px] transition-all flex items-center justify-center gap-2 ${formData.productId && !isImporting ? 'bg-indigo-600 text-white shadow-lg hover:bg-indigo-700' : 'bg-gray-100 text-gray-400'}`}>
                ...
                <tr key={item.id} className={`hover:bg-gray-50 transition-colors ${item.paid ? 'bg-green-50/40 italic text-gray-400' : ''} ${editingLogId === item.id ? 'bg-indigo-50' : ''}`}>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    <div className="flex items-center justify-center gap-1">
                      <button onClick={() => handleEditClick(item)} className="text-gray-300 hover:text-indigo-600 p-1 rounded transition-colors" title="Editar"><Pencil size={14}/></button>
                      <button onClick={() => onDelete(item.id)} className="text-gray-300 hover:text-red-500 p-1 rounded transition-colors" title="Excluir"><Trash2 size={14}/></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {showPendingModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl z-10 overflow-hidden animate-in zoom-in duration-200">
            <div className="bg-indigo-600 p-6 text-white flex justify-between items-center">
              <div>
                <h3 className="text-lg font-bold flex items-center gap-2"><ShoppingBag size={20} /> Novos Produtos Encontrados</h3>
                <p className="text-xs text-indigo-100 mt-1 opacity-90">Processando {importedFilesCount} arquivos XML.</p>
              </div>
              <button onClick={() => !isImporting && setShowPendingModal(false)} className="hover:bg-white/10 p-2 rounded-full transition-all"><X size={20} /></button>
            </div>
            <div className="p-6 max-h-[60vh] overflow-y-auto">
              <div className="mb-4 bg-gray-50 p-3 rounded-lg flex items-center gap-3 border border-indigo-100">
                 <Files className="text-indigo-500" size={18} />
                 <span className="text-xs font-bold text-gray-600">Total de {allEntriesToLog.length} lançamentos pendentes em lote.</span>
              </div>
              <table className="w-full">
                <thead className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  <tr className="border-b"><th className="pb-3 text-left">Produto Novo</th><th className="pb-3 text-right">Venda</th><th className="pb-3 text-center">Mão de Obra (R$)</th></tr>
                </thead>
                <tbody className="divide-y">
                  {pendingProducts.map((item, idx) => (
                    <tr key={idx} className="group">
                      <td className="py-4 pr-4"><span className="text-xs font-bold text-gray-800">{item.name}</span></td>
                      <td className="py-4 text-right text-xs font-bold text-gray-600">R$ {item.manufacturingValue.toFixed(2)}</td>
                      <td className="py-4 pl-6 text-center">
                        <input type="number" step="0.01" placeholder="0,00" value={item.laborCost || ''} onChange={(e) => { const newItems = [...pendingProducts]; newItems[idx].laborCost = parseFloat(e.target.value) || 0; setPendingProducts(newItems); }} className="w-24 text-center py-1.5 border-2 border-indigo-50 rounded-lg text-xs font-bold outline-none focus:border-indigo-500 transition-all" />
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="p-6 bg-gray-50 border-t flex justify-end gap-3">
              ...
              <button onClick={handleConfirmPending} disabled={isImporting || pendingProducts.some(i => i.laborCost <= 0)} className={`px-8 py-2 text-xs font-bold rounded-xl text-white transition-all flex items-center gap-2 ${isImporting || pendingProducts.some(i => i.laborCost <= 0) ? 'bg-gray-300 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 active:scale-95 shadow-indigo-200 shadow-lg'}`}>
                {isImporting ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} 
                {isImporting ? 'Processando...' : 'Confirmar Lançamentos'}
              </button>
            </div>
          </div>
        </div>
